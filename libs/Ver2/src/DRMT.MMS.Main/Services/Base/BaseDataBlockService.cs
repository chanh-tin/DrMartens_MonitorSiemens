...
{
    public class BaseDataBlockService : BaseCRUDService<DataBlockEntity>
    {
        protected CommonDBContext _dbContextImp;
        protected BaseDataBlockRepository _repositoryBase;
        protected TagRepository _tagRepository;
        protected PlcRepository _plcRepository;
        

        public BaseDataBlockService()
        {
        }
        public BaseDataBlockService(
            ...
            : base(dbContext, logger)
        {
            ...
            _repositoryCRUD = new BaseDataBlockRepository(dbContext);
            _repositoryBase = (BaseDataBlockRepository)_repositoryCRUD;
        ...
        {
            return nameof(BaseDataBlockService);
        }
            ...
            bool isTracking = false)
        {
            ...
            List<DataBlockResponseModel> listItemResponse = new DataBlockResponseModel().SetData(list).Cast<DataBlockResponseModel>().ToList();

            //* get column language
            var columns = new DataBlockResponseModel().GetColumnAttribute();

            this.TranslateData(columns, request.Lang, nameof(DataBlockEntity));
            ...
            using (var outStream = new MemoryStream())
            {
                using (var archive = new ZipArchive(outStream, ZipArchiveMode.Create, true))
                {
                    ...
                    using (var report = ExcelPro.LoadReportFormat())
                    {
                        ...
                        var listResponseModel = new BaseDataBlockResponseModel().SetData(list).Cast<BaseDataBlockResponseModel>().ToList();
                        ...
                        foreach (var item in listResponseModel)
                        {
                            ...
                            foreach (PropertyInfo property in typeof(BaseDataBlockResponseModel).GetProperties().OrderBy(p => p.Name))
                            {
        ...
        public override List<DataBlockEntity> GetAll()
        {
            var list = base.GetAll();
            return list;
        }
        public override List<Dictionary<string, object>> GetFormDataObjElement(DataBlockEntity entity,
            string lang = null,
            List<string> translateFields = null)
        {
            string entityName = nameof(DataBlockEntity);
            var listRS = new List<Dictionary<string, object>>();
            List<object> objectList = null;
            var properties = typeof(DataBlockEntity).GetProperties();
            ...
            var dicFieldName2Index = new DataBlockResponseModel().GetColumnAttribute()
        ...
        /// <returns></returns>
        public override DataBlockEntity Upsert(DataBlockEntity entity, long? userId = null)
        {
            List<TagEntity> tagChildren = null;
            if (entity.TagIds != null && entity.TagIds.Count >= 1)
            {
                tagChildren = _tagRepository.GetListByListIds(entity.TagIds, true, true);
                if (tagChildren == null || tagChildren.Count <= 0)
                {
                    throw new DBException($"tagChildren not found, ids: {string.Join(",", entity.TagIds)}");
                }
            }

            List<PlcEntity> plcChildren = null;
            if (entity.PlcIds != null && entity.PlcIds.Count >= 1)
            {
                plcChildren = _plcRepository.GetListByListIds(entity.PlcIds, true, true);
                if (plcChildren == null || plcChildren.Count <= 0)
                {
                    throw new DBException($"plcChildren not found, ids: {string.Join(",", entity.PlcIds)}");
                }
            }
            
            ((BaseDataBlockRepository)_repositoryBase).Upsert(entity, tagChildren, plcChildren, userId);

            var upsertedEntity = ((BaseDataBlockRepository)_repositoryBase).GetById(entity.Id, false);
        ...
        public override int Delete(DataBlockEntity entity, long? userId = null, bool isSoftDelete = true)
        {
        ...
        /// <returns></returns>
        public virtual List<long> GetListIdChildren(DataBlockEntity entity, string childEntity)
        {
            switch (childEntity)
            {
                case nameof(TagEntity):
                    ...
                    return entity.ListTag.Select(x => x.Id).ToList();

                case nameof(PlcEntity):
                    if (entity.ListPlc == null)
                    {
                        return new List<long>();
                    }
                    return entity.ListPlc.Select(x => x.Id).ToList();
        ...
        /// <param name="childEntity"></param>
        /// <returns></returns>
        public virtual long? GetItemIdChildren(DataBlockEntity entity, string childEntity)
        {
            switch (childEntity)
            {
        ...
        public virtual List<FormSelectOptionModel> GetListOptionData(string targetEntity, string entityName, EnumAttributeRelationshipType category = EnumAttributeRelationshipType.Many2Many)
        {
            ...
            {
                case nameof(TagEntity):
                        listRS = this._tagRepository.GetSelectData(entityName, category);
                        break;

                    case nameof(PlcEntity):
                        listRS = this._plcRepository.GetSelectData(entityName, category);
                        break;
        ...
        public override DataBlockEntity SetFileURL(DataBlockEntity entity, Dictionary<string, string> dicImagePath)
        {
        ...
        //public DataBlockEntity UpsertTestTransaction(DataBlockEntity item, long? userId = null)
        //{
