...
namespace SourceBaseBE.Database.Repository
{
	public class HolidayWorkingTypeRepository : BaseCRUDRepository<HolidayWorkingTypeEntity>
	{
		...
		public override HolidayWorkingTypeEntity? GetById(long id, bool isTracking = false)
		{
			try
			{
    ...
    //    {
          ...
          result = queryable
                ...
                .Where(entity => entity.DeletedFlag != true && entity.Id == id)
							  .FirstOrDefault();
					//result.HolidayWorkingType2s = result.HolidayWorkingType2s.Select(x => new HolidayWorkingType2Entity() { Id = x.Id, LossName = x.LossName }).ToList();
		...
		/// <returns></returns>
		/// <exception cref="DBException"></exception>
		public async Task<HolidayWorkingTypeEntity>? GetByIdAsync(long id, bool isDirect = false, bool isTracking = true)
		{
			try
			{
				HolidayWorkingTypeEntity? result = null;
		...
		/// <returns></returns>
		/// <exception cref="DBException"></exception>
		public override List<HolidayWorkingTypeEntity> GetList(PagingRequestModel pagingReq = null, bool isDirect = false, bool isTracking = false)
		{
			try
			{
				...
				if (result == null)
				{
					...
					if (pagingReq != null)
					{
						...
						// {
						//   Id = entity.Id,
						//   HolidayWorkingType2s = entity.HolidayWorkingType2s.Select(x => new HolidayWorkingType2Entity { Id = x.Id, LossName = x.LossName }).ToList()
						// })
...
/*[GEN-9]*/
		...
		public override object GetDisplayField(HolidayWorkingTypeEntity entity)
		{
