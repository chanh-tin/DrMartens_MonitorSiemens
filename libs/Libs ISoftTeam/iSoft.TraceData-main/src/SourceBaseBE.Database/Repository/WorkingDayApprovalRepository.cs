...
namespace SourceBaseBE.Database.Repository
{
	public class WorkingDayApprovalRepository : BaseCRUDRepository<WorkingDayApprovalEntity>
	{
		...
		public override WorkingDayApprovalEntity? GetById(long id, bool isTracking = false)
		{
			try
			{
    ...
    //    {
          ...
          result = queryable
                ...
                .Where(entity => entity.DeletedFlag != true && entity.Id == id)
							  .FirstOrDefault();
					//result.WorkingDayApproval2s = result.WorkingDayApproval2s.Select(x => new WorkingDayApproval2Entity() { Id = x.Id, LossName = x.LossName }).ToList();
		...
		/// <returns></returns>
		/// <exception cref="DBException"></exception>
		public async Task<WorkingDayApprovalEntity>? GetByIdAsync(long id, bool isDirect = false, bool isTracking = true)
		{
			try
			{
				WorkingDayApprovalEntity? result = null;
		...
		/// <returns></returns>
		/// <exception cref="DBException"></exception>
		public override List<WorkingDayApprovalEntity> GetList(PagingRequestModel pagingReq = null, bool isDirect = false, bool isTracking = false)
		{
			try
			{
				...
				if (result == null)
				{
					...
					if (pagingReq != null)
					{
						...
						// {
						//   Id = entity.Id,
						//   WorkingDayApproval2s = entity.WorkingDayApproval2s.Select(x => new WorkingDayApproval2Entity { Id = x.Id, LossName = x.LossName }).ToList()
						// })
		...
		public override object GetDisplayField(WorkingDayApprovalEntity entity)
		{
