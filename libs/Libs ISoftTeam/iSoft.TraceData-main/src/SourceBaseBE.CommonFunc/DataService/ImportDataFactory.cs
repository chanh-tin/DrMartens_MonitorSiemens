...
namespace SourceBaseBE.CommonFunc.DataService
{
    public class ImportDataFactory : IDisposable
    {
        ...
        protected InfluxDBService _influxDBService;
        ...
        private void saveTraceData(DevicePayloadMessage message, EnumDBProvider databaseType)
        {
            ...
            if (!cached.ContainsKey($"log_tracedata_message_seconds_5m_all"))
            {
                cached.AddToCache($"log_tracedata_message_seconds_5m_all", true, 60 * 10);
                this._logger.LogMsg(Messages.IBegin_0_1, $"_5m_ [tracedata5m] All", message.MessageId + ", message: " + message.ToJson());
            }
        ...
        public void ImportInfluxDB(DevicePayloadMessage message)
        {
            const string funcName = nameof(ImportInfluxDB);
            if (!cached.ContainsKey($"log_{funcName}_message_seconds_5m_all"))
            {
                cached.AddToCache($"log_{funcName}_message_seconds_5m_all", true, 60 * 5);
                this._logger.LogMsg(Messages.IBegin_0_1, $"_5m_ [{funcName}5m] All", message.MessageId + ", message: " + message.ToJson());
            }

            Dictionary<string, TraceDataTableObj> listTable = getListTraceDataTableObj(message);

            string keyLock = $"saveTraceData_{funcName}_{message.MessageId}";
            if (CachedSupportFunc.RequireLockAndRetry(keyLock, 1000, 12, 8))
            {
                try
                {
                    foreach (var key in listTable.Keys)
                    {
                        ...
                        try
                        {
                            if (!cached.ContainsKey($"log_{funcName}_message_seconds_5m_{val.TableName}"))
                            {
                                cached.AddToCache($"log_{funcName}_message_seconds_5m_{val.TableName}", true, 60 * 5);
                                this._logger.LogMsg(Messages.IBegin_0_1, $"_5m_ [{funcName}5m], {val.TableName}", message.MessageId + ", values: " + val.DicEnvKey2Value.ToJson());
                            }
                            var ret = val.InsertInfluxDB(this._influxDBService, ref dicTableName2LastData);
                            if (ret == EnumFuncResult.Error)
                            {
                                return;
                            }
            ...
            {
                this._logger.LogInformation($"[{funcName}] Check locked item (Duplicate), MessageId: {message.MessageId}");
                throw new RetryException($"[{funcName}] Check locked item (Duplicate), MessageId: {message.MessageId}");
            }
        }
