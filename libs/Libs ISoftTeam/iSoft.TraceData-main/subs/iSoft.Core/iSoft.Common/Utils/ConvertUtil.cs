...
namespace iSoft.Common.Utils
{
    public class ConvertUtil
    {
        ...
        public static List<T> RemoveDuplicateBy<T>(List<T> list, Func<T, object> func)
        {
            return list.DistinctBy(func).ToList();
        }
        ...
        public static object? ConvertData(object value, string fieldType)
        {
            if (fieldType == typeof(long).ToString())
            {
                return ConvertToNullableLong(value);
            }
            else if (fieldType == typeof(int).ToString())
            {
                return ConvertToNullableInt(value);
            }
            else if (fieldType == typeof(short).ToString())
            {
                return ConvertToNullableShort(value);
            }
            else if (fieldType == typeof(byte).ToString())
            {
                return ConvertToNullableByte(value);
            }
            else if (fieldType == typeof(double).ToString())
            {
                return ConvertToNullableDouble(value);
            }
            else if (fieldType == typeof(bool).ToString())
            {
                return ConvertToNullableBool(value);
            }
            else if (fieldType == typeof(DateTime).ToString())
            {
                return ConvertToNullableDateTime(value);
            }
            else if (fieldType == typeof(string).ToString())
            {
                return value.ToString();
            }
            return value;
