...
{
    public class BaseGenTemplateService : BaseCRUDService<GenTemplateEntity>
    {
        protected UserRepository _userRepository;
        protected BaseGenTemplateRepository _repositoryBase;
        /*[GEN-1]*/

        public BaseGenTemplateService()
        {
        }
        public BaseGenTemplateService(CommonDBContext dbContext, ILogger<BaseGenTemplateService> logger)
        {
            _dbContext = dbContext;
            _logger = logger;
            _repositoryCRUD = new BaseGenTemplateRepository(_dbContext);
            _repositoryBase = (BaseGenTemplateRepository)_repositoryCRUD;
        ...
        public override GenTemplateEntity GetById(long id, bool isTracking = false)
        {
            ...
            var entityRS = (GenTemplateEntity)_userRepository.FillTrackingUser(entity);
            return entityRS;
        }
        public override List<GenTemplateEntity> GetList(PagingRequestModel pagingReq = null, bool isDirect = false, bool isTracking = false)
        {
            ...
            var listRS = _userRepository.FillTrackingUser(list.Cast<BaseCRUDEntity>().ToList()).Cast<GenTemplateEntity>().ToList();
            return listRS;
        }
            ...
            bool isTracking = false)
        {
            ...
            var list = _userRepository.FillTrackingUser(list0.Cast<BaseCRUDEntity>().ToList()).Cast<GenTemplateEntity>().ToList();

            PagingWithColumnsResponseModel rs = new PagingWithColumnsResponseModel();
            List<BaseGenTemplateResponseModel> listItemResponse = new BaseGenTemplateResponseModel().SetData(list).Cast<BaseGenTemplateResponseModel>().ToList();
            
            rs.ListData = listItemResponse.Cast<object>().ToList();
            rs.TotalRecord = totalRecord;
            rs.Columns = new BaseGenTemplateResponseModel().GetColumnAttribute();
            ...
            using (var outStream = new MemoryStream())
            {
                using (var archive = new ZipArchive(outStream, ZipArchiveMode.Create, true))
                {
                    ...
                    using (var report = ExcelPro.LoadReportFormat())
                    {
                        ...
                        var list = _userRepository.FillTrackingUser(list0.Cast<BaseCRUDEntity>().ToList()).Cast<GenTemplateEntity>().ToList();
                        var listResponseModel = new BaseGenTemplateResponseModel().SetData(list).Cast<BaseGenTemplateResponseModel>().ToList();
                        ...
                        foreach (var item in listResponseModel)
                        {
                            ...
                            foreach (PropertyInfo property in typeof(BaseGenTemplateResponseModel).GetProperties().OrderBy(p => p.Name))
                            {
        ...
        public override List<GenTemplateEntity> GetAll()
        {
            var list = _repositoryBase.GetAll();
            var listRS = _userRepository.FillTrackingUser(list.Cast<BaseCRUDEntity>().ToList()).Cast<GenTemplateEntity>().ToList();
            return listRS;
        }
        public override List<Dictionary<string, object>> GetFormDataObjElement(GenTemplateEntity entity)
        {
            string entityName = nameof(GenTemplateEntity);
            var listRS = new List<Dictionary<string, object>>();
            List<object> objectList = null;
            var properties = typeof(GenTemplateEntity).GetProperties();
        ...
        /// <returns></returns>
        public override GenTemplateEntity Upsert(GenTemplateEntity entity, long? userId = null)
        {
            /*[GEN-3]*/
            ((BaseGenTemplateRepository)_repositoryBase).Upsert(entity/*[GEN-4]*/, userId);

            var upsertedEntity = ((BaseGenTemplateRepository)_repositoryBase).GetById(entity.Id, false);

            var entityRS = (GenTemplateEntity)_userRepository.FillTrackingUser(upsertedEntity);
            return entityRS;
        }
        ...
        /// <returns></returns>
        public List<long> GetListIdChildren(GenTemplateEntity entity, string childEntity)
        {
            switch (childEntity)
            {
        ...
        public override GenTemplateEntity SetFileURL(GenTemplateEntity entity, Dictionary<string, string> dicImagePath)
        {
        ...
        //public GenTemplateEntity UpsertTestTransaction(GenTemplateEntity item, long? userId = null)
        //{
