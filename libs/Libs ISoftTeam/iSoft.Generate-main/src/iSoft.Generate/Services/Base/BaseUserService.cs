...
{
    public class BaseUserService : BaseCRUDService<UserEntity>
    {
        protected UserRepository _userRepository;
        protected BaseUserRepository _repositoryBase;
        

        public BaseUserService()
        {
        }
        public BaseUserService(CommonDBContext dbContext, ILogger<BaseUserService> logger)
        {
            _dbContext = dbContext;
            _logger = logger;
            _repositoryCRUD = new BaseUserRepository(_dbContext);
            _repositoryBase = (BaseUserRepository)_repositoryCRUD;
        ...
        public override List<UserEntity> GetList(PagingRequestModel pagingReq = null, bool isDirect = false, bool isTracking = false)
        {
            ...
            var listRS = _userRepository.FillTrackingUser(list.Cast<BaseCRUDEntity>().ToList()).Cast<UserEntity>().ToList();
            return listRS;
        }
            ...
            bool isTracking = false)
        {
            ...
            var list = _userRepository.FillTrackingUser(list0.Cast<BaseCRUDEntity>().ToList()).Cast<UserEntity>().ToList();

            PagingWithColumnsResponseModel rs = new PagingWithColumnsResponseModel();
            List<BaseUserResponseModel> listItemResponse = new BaseUserResponseModel().SetData(list).Cast<BaseUserResponseModel>().ToList();
            
            rs.ListData = listItemResponse.Cast<object>().ToList();
            rs.TotalRecord = totalRecord;
            rs.Columns = new BaseUserResponseModel().GetColumnAttribute();
            ...
            using (var outStream = new MemoryStream())
            {
                using (var archive = new ZipArchive(outStream, ZipArchiveMode.Create, true))
                {
                    ...
                    using (var report = ExcelPro.LoadReportFormat())
                    {
                        ...
                        var list = _userRepository.FillTrackingUser(list0.Cast<BaseCRUDEntity>().ToList()).Cast<UserEntity>().ToList();
                        var listResponseModel = new BaseUserResponseModel().SetData(list).Cast<BaseUserResponseModel>().ToList();
                        ...
                        foreach (var item in listResponseModel)
                        {
                            ...
                            foreach (PropertyInfo property in typeof(BaseUserResponseModel).GetProperties().OrderBy(p => p.Name))
                            {
        ...
        {
            ((BaseUserRepository)_repositoryBase).Upsert(entity, userId);

            var upsertedEntity = ((BaseUserRepository)_repositoryBase).GetById(entity.Id, false);

            var entityRS = (UserEntity)_userRepository.FillTrackingUser(upsertedEntity);
            return entityRS;
        }
        ...
        /// <returns></returns>
        public List<long> GetListIdChildren(UserEntity entity, string childEntity)
        {
            switch (childEntity)
            {
                ...
                case nameof(PermissionEntity):
                    if (entity.ListPermission == null)
                    {
                        return new List<long>();
                    }
                    return entity.ListPermission.Select(x => x.Id).ToList();

                case nameof(UserGroupEntity):
                    if (entity.ListUserGroup == null)
                    {
                        return new List<long>();
                    }
                    return entity.ListUserGroup.Select(x => x.Id).ToList();
        ...
        public override UserEntity SetFileURL(UserEntity entity, Dictionary<string, string> dicImagePath)
        {
            if (dicImagePath != null && dicImagePath.Count >= 1)
            {
                if (dicImagePath.ContainsKey(nameof(UserEntity.Avatar)))
                {
                    entity.Avatar = dicImagePath[nameof(UserEntity.Avatar)];
                }
        ...
        //public UserEntity UpsertTestTransaction(UserEntity item, long? userId = null)
        //{
