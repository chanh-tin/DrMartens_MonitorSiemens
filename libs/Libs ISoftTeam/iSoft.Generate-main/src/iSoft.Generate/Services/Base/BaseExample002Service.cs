...
{
    public class BaseExample002Service : BaseCRUDService<Example002Entity>
    {
        protected UserRepository _userRepository;
        protected BaseExample002Repository _repositoryBase;
        protected Example001Repository _example001Repository;
        

        public BaseExample002Service()
        {
        }
        public BaseExample002Service(CommonDBContext dbContext, ILogger<BaseExample002Service> logger)
        {
            _dbContext = dbContext;
            _logger = logger;
            _repositoryCRUD = new BaseExample002Repository(_dbContext);
            _repositoryBase = (BaseExample002Repository)_repositoryCRUD;
        ...
        public override Example002Entity GetById(long id, bool isTracking = false)
        {
            ...
            var entityRS = (Example002Entity)_userRepository.FillTrackingUser(entity);
            return entityRS;
        }
        public override List<Example002Entity> GetList(PagingRequestModel pagingReq = null, bool isDirect = false, bool isTracking = false)
        {
            ...
            var listRS = _userRepository.FillTrackingUser(list.Cast<BaseCRUDEntity>().ToList()).Cast<Example002Entity>().ToList();
            return listRS;
        }
            ...
            bool isTracking = false)
        {
            ...
            var list = _userRepository.FillTrackingUser(list0.Cast<BaseCRUDEntity>().ToList()).Cast<Example002Entity>().ToList();

            PagingWithColumnsResponseModel rs = new PagingWithColumnsResponseModel();
            List<BaseExample002ResponseModel> listItemResponse = new BaseExample002ResponseModel().SetData(list).Cast<BaseExample002ResponseModel>().ToList();
            
            rs.ListData = listItemResponse.Cast<object>().ToList();
            rs.TotalRecord = totalRecord;
            rs.Columns = new BaseExample002ResponseModel().GetColumnAttribute();
            ...
            using (var outStream = new MemoryStream())
            {
                using (var archive = new ZipArchive(outStream, ZipArchiveMode.Create, true))
                {
                    ...
                    using (var report = ExcelPro.LoadReportFormat())
                    {
                        ...
                        var list = _userRepository.FillTrackingUser(list0.Cast<BaseCRUDEntity>().ToList()).Cast<Example002Entity>().ToList();
                        var listResponseModel = new BaseExample002ResponseModel().SetData(list).Cast<BaseExample002ResponseModel>().ToList();
                        ...
                        foreach (var item in listResponseModel)
                        {
                            ...
                            foreach (PropertyInfo property in typeof(BaseExample002ResponseModel).GetProperties().OrderBy(p => p.Name))
                            {
        ...
        public override List<Example002Entity> GetAll()
        {
            var list = _repositoryBase.GetAll();
            var listRS = _userRepository.FillTrackingUser(list.Cast<BaseCRUDEntity>().ToList()).Cast<Example002Entity>().ToList();
            return listRS;
        }
        public override List<Dictionary<string, object>> GetFormDataObjElement(Example002Entity entity)
        {
            string entityName = nameof(Example002Entity);
            var listRS = new List<Dictionary<string, object>>();
            List<object> objectList = null;
            var properties = typeof(Example002Entity).GetProperties();
        ...
        /// <returns></returns>
        public override Example002Entity Upsert(Example002Entity entity, long? userId = null)
        {
            ...
            ((BaseExample002Repository)_repositoryBase).Upsert(entity, example001Children, userId);

            var upsertedEntity = ((BaseExample002Repository)_repositoryBase).GetById(entity.Id, false);

            var entityRS = (Example002Entity)_userRepository.FillTrackingUser(upsertedEntity);
            return entityRS;
        }
        ...
        /// <returns></returns>
        public List<long> GetListIdChildren(Example002Entity entity, string childEntity)
        {
            switch (childEntity)
            {
        ...
        public override Example002Entity SetFileURL(Example002Entity entity, Dictionary<string, string> dicImagePath)
        {
        ...
        //public Example002Entity UpsertTestTransaction(Example002Entity item, long? userId = null)
        //{
