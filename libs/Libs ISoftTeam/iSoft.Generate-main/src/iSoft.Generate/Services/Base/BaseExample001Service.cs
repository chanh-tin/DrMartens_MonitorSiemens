...
namespace SourceBaseBE.MainService.Services
{
    public class BaseExample001Service : BaseCRUDService<Example001Entity>
    {
        ...
        public BaseExample001Service(CommonDBContext dbContext, ILogger<BaseExample001Service> logger)
        {
            _dbContext = dbContext;
            _logger = logger;
            _repositoryCRUD = new BaseExample001Repository(_dbContext);
        ...
        public override Example001Entity GetById(long id, bool isTracking = false)
        {
            ...
            var entityRS = (Example001Entity)_userRepository.FillTrackingUser(entity);
            return entityRS;
        }
        public override List<Example001Entity> GetList(PagingRequestModel pagingReq = null, bool isDirect = false, bool isTracking = false)
        {
            ...
            var listRS = _userRepository.FillTrackingUser(list.Cast<BaseCRUDEntity>().ToList()).Cast<Example001Entity>().ToList();
            return listRS;
        }
            ...
            bool isTracking = false)
        {
            ...
            var list = _userRepository.FillTrackingUser(list0.Cast<BaseCRUDEntity>().ToList()).Cast<Example001Entity>().ToList();

            PagingWithColumnsResponseModel rs = new PagingWithColumnsResponseModel();
            List<BaseExample001ResponseModel> listItemResponse = new BaseExample001ResponseModel().SetData(list).Cast<BaseExample001ResponseModel>().ToList();
            
            rs.ListData = listItemResponse.Cast<object>().ToList();
            rs.TotalRecord = totalRecord;
            rs.Columns = new BaseExample001ResponseModel().GetColumnAttribute();
        ...
        public override Example001Entity Upsert(Example001Entity entity, long? userId = null)
        {
            ...
            var entityRS = (Example001Entity)_userRepository.FillTrackingUser(upsertedEntity);
            return entityRS;
        }
        ...
        /// <returns></returns>
        public List<long> GetListIdChildren(Example001Entity entity, string childEntity)
        {
            switch (childEntity)
            {
