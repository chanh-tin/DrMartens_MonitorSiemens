...
namespace iSoft.Extensions.EntityFrameworkCore.Repository
{
    ...
    public class GenericRepository<TContext, TEntity> : IDisposable, IGenericRepository<TEntity> where TEntity : BaseEntity where TContext : DbContext, new()
    {
        ...
        public virtual Task<List<TEntity>> GetList(PagingFilterRequestModel request = null)
        {
            if (request == null)
            {
                request = new PagingFilterRequestModel();
            }
            if (request.Page <= 0)
            {
                request.Page = 1;
            ...
            return Context.Set<TEntity>().Skip(request.Page).Take(request.PageSize).ToListAsync();
        }
    }
}
