...
{
    public class BasePlcService : BaseCRUDService<PlcEntity>
    {
        protected CommonDBContext _dbContextImp;
        protected BasePlcRepository _repositoryBase;
        protected DataBlockRepository _dataBlockRepository;
        

        public BasePlcService()
        {
        }
        public BasePlcService(
            ...
            : base(dbContext, logger)
        {
            ...
            _repositoryCRUD = new BasePlcRepository(dbContext);
            _repositoryBase = (BasePlcRepository)_repositoryCRUD;
            this._dataBlockRepository = new DataBlockRepository(dbContext);
            
        }
        public override string GetServiceName()
        {
            return nameof(BasePlcService);
        }
            ...
            bool isTracking = false)
        {
            ...
            List<PlcResponseModel> listItemResponse = new PlcResponseModel().SetData(list).Cast<PlcResponseModel>().ToList();

            //* get column language
            var columns = new PlcResponseModel().GetColumnAttribute();

            this.TranslateData(columns, request.Lang, nameof(PlcEntity));
            ...
            using (var outStream = new MemoryStream())
            {
                using (var archive = new ZipArchive(outStream, ZipArchiveMode.Create, true))
                {
                    ...
                    using (var report = ExcelPro.LoadReportFormat())
                    {
                        ...
                        var listResponseModel = new BasePlcResponseModel().SetData(list).Cast<BasePlcResponseModel>().ToList();
                        ...
                        foreach (var item in listResponseModel)
                        {
                            ...
                            foreach (PropertyInfo property in typeof(BasePlcResponseModel).GetProperties().OrderBy(p => p.Name))
                            {
        ...
        public override List<PlcEntity> GetAll()
        {
            var list = base.GetAll();
            return list;
        }
        public override List<Dictionary<string, object>> GetFormDataObjElement(PlcEntity entity,
            string lang = null,
            List<string> translateFields = null)
        {
            string entityName = nameof(PlcEntity);
            var listRS = new List<Dictionary<string, object>>();
            List<object> objectList = null;
            var properties = typeof(PlcEntity).GetProperties();
            ...
            var dicFieldName2Index = new PlcResponseModel().GetColumnAttribute()
        ...
        /// <returns></returns>
        public override PlcEntity Upsert(PlcEntity entity, long? userId = null)
        {
            List<DataBlockEntity> dataBlockChildren = null;
            if (entity.DataBlockIds != null && entity.DataBlockIds.Count >= 1)
            {
                dataBlockChildren = _dataBlockRepository.GetListByListIds(entity.DataBlockIds, true, true);
                if (dataBlockChildren == null || dataBlockChildren.Count <= 0)
                {
                    throw new DBException($"dataBlockChildren not found, ids: {string.Join(",", entity.DataBlockIds)}");
                }
            }
            
            ((BasePlcRepository)_repositoryBase).Upsert(entity, dataBlockChildren, userId);

            var upsertedEntity = ((BasePlcRepository)_repositoryBase).GetById(entity.Id, false);
        ...
        public override int Delete(PlcEntity entity, long? userId = null, bool isSoftDelete = true)
        {
        ...
        /// <returns></returns>
        public virtual List<long> GetListIdChildren(PlcEntity entity, string childEntity)
        {
            switch (childEntity)
            {
        ...
        /// <param name="childEntity"></param>
        /// <returns></returns>
        public virtual long? GetItemIdChildren(PlcEntity entity, string childEntity)
        {
            switch (childEntity)
            {
        ...
        public override PlcEntity SetFileURL(PlcEntity entity, Dictionary<string, string> dicImagePath)
        {
        ...
        //public PlcEntity UpsertTestTransaction(PlcEntity item, long? userId = null)
        //{
