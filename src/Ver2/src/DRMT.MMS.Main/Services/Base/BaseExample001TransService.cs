...
{
    public class BaseExample001TransService : BaseCRUDService<Example001TransEntity>
    {
        protected CommonDBContext _dbContextImp;
        protected BaseExample001TransRepository _repositoryBase;
        protected Example001Repository _example001Repository;
        

        public BaseExample001TransService()
        {
        }
        public BaseExample001TransService(
            ...
            : base(dbContext, logger)
        {
            ...
            _repositoryCRUD = new BaseExample001TransRepository(dbContext);
            _repositoryBase = (BaseExample001TransRepository)_repositoryCRUD;
            this._example001Repository = new Example001Repository(dbContext);
            
        }
        public override string GetServiceName()
        {
            return nameof(BaseExample001TransService);
        }
            ...
            bool isTracking = false)
        {
            ...
            List<Example001TransResponseModel> listItemResponse = new Example001TransResponseModel().SetData(list).Cast<Example001TransResponseModel>().ToList();

            //* get column language
            var columns = new Example001TransResponseModel().GetColumnAttribute();

            this.TranslateData(columns, request.Lang, nameof(Example001TransEntity));
            ...
            using (var outStream = new MemoryStream())
            {
                using (var archive = new ZipArchive(outStream, ZipArchiveMode.Create, true))
                {
                    ...
                    using (var report = ExcelPro.LoadReportFormat())
                    {
                        ...
                        var listResponseModel = new BaseExample001TransResponseModel().SetData(list).Cast<BaseExample001TransResponseModel>().ToList();
                        ...
                        foreach (var item in listResponseModel)
                        {
                            ...
                            foreach (PropertyInfo property in typeof(BaseExample001TransResponseModel).GetProperties().OrderBy(p => p.Name))
                            {
        ...
        public override List<Example001TransEntity> GetAll()
        {
            var list = base.GetAll();
            return list;
        }
        public override List<Dictionary<string, object>> GetFormDataObjElement(
            Example001TransEntity entity,
            string lang = null,
            List<string> translateFields = null
            )
        {
            string entityName = nameof(Example001TransEntity);
            var listRS = new List<Dictionary<string, object>>();
            List<object> objectList = null;
            var properties = typeof(Example001TransEntity).GetProperties();
            ...
            var dicFieldName2Index = new Example001TransResponseModel().GetColumnAttribute()
        ...
        /// <returns></returns>
        public override Example001TransEntity Upsert(Example001TransEntity entity, long? userId = null)
        {
            
            ((BaseExample001TransRepository)_repositoryBase).Upsert(entity, userId);

            var upsertedEntity = ((BaseExample001TransRepository)_repositoryBase).GetById(entity.Id, false);
        ...
        public override int Delete(Example001TransEntity entity, long? userId = null, bool isSoftDelete = true)
        {
        ...
        /// <returns></returns>
        public virtual List<long> GetListIdChildren(Example001TransEntity entity, string childEntity)
        {
            switch (childEntity)
            {
                ...
                if (result == null)
                {
                    ...
                    foreach (var column in columns)
                    {
                        if (column.Filterable != null && column.Filterable.Value == true)
                        {
                            switch (column.Key)
                            {
                                //case nameof(Example001TransResponseModel.BaseName):
                                //    listOption = _example001Repository.GetListWithNoInclude().Select(x =>
                                //        new OptionFilterResponseModel(x.Id.ToString(), _example001Repository.GetDisplayField(x).ToString())).ToList();
                    ...
                    CachedFunc.AddEntityCacheKey(_example001Repository.GetRepositoryName(), cacheKey, false);
        ...
        public override Example001TransEntity SetFileURL(Example001TransEntity entity, Dictionary<string, string> dicImagePath)
        {
        ...
        //public Example001TransEntity UpsertTestTransaction(Example001TransEntity item, long? userId = null)
        //{
